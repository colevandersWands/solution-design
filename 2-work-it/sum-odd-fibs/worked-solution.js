{
	...
	*/
	...
	console.log('--- loggify solution ---');
		...
		test_cases.push({name: '4 -> 10', args: [4], expected: 10});
		test_cases.push({name: '6 -> 5', args: [6], expected: 5});

		function sum_odd_fibs_lesseq_arg_logged(_num, _log) {
			let log;
			if (_log)   log = { args: {_num} };

			const arr_o_fibs = find_fibs_lesseq_arg(_num);
			if (_log)   log.arr_o_fibs = arr_o_fibs;

			const odds_only = select_odds(arr_o_fibs);
			if (_log)   log.odds_only = odds_only;
		...
		run_tests(sum_odd_fibs_lesseq_arg_logged, test_cases, true);
