{
  ...
  */
  ...
  console.log('--- log strategy ---');
    ...
    function sum_odd_fibs_lesseq_arg_logg(_num) {     const log = [{arg: _num}];
      const arr_o_fibs = find_fibs_lesseq_arg(_num);  log.push({arr_o_fibs});

      const odds_only = select_odds(arr_o_fibs);      log.push({odds_only});

      const sum = sum_array(odds_only);               log.push({sum});

      return {result: sum, log};
    };
    log(sum_odd_fibs_lesseq_arg_logg, test_cases);
